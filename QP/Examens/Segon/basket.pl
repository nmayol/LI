...
satVariable( triple(S,R)  ):- team(S),          round(R).   %  "team S has a triple on round R"
...
findDoubles:-
    team(T),
    round(R), R \= 1, R1 is R - 1,
    writeClause([-home(T,R),-home(T,R1),double(T,R)]),
    writeClause([home(T,R),home(T,R1),double(T,R)]),
    fail.
findDoubles.
...
noDoublesRounds:-
    ...
    noDoubles(Ds),
    member(R,Ds),
    findall(double(S,R),team(S),Lits),
    exactly(0,Lits),
    fail.
noDoublesRounds.
...
notTeamsHome:-
    ...
    notHome(T,Hs),
    member(R,Hs),
    writeClause([-home(T,R)]),
    fail.
notTeamsHome.

atLeastOneTV:-
    round(R),
    findall(match(S,T,R),(team(S),team(T),tvMatch(S,T)),Lits),
    atLeast(1,Lits),
    fail.
atLeastOneTV.
...
noTriples:-
    ...
    round(R), R > 2,
    R1 is R-1, R2 is R-2,
    writeClause([-home(T,R1),-home(T,R2),-home(T,R)]),
    writeClause([home(T,R1),home(T,R2),home(T,R)]),
    fail.
noTriples.
