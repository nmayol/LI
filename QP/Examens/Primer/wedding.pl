...
% Serveix per formar els grups (a la linia 15 esta inicialitzat a 4).

allTablesGuests:-
    minPeopleAtTable(N),
    maxPeopleAtTable(M),
    table(T),
    findall(guestTable(G,T),guest(G),GuestsTable),
    atLeast(N,GuestsTable),
    atMost(M,GuestsTable),
    fail.
allTablesGuests.

everyGuestJustOneTable:-
    guest(G),
    findall(guestTable(G,T),table(T),Tables),
    exactly(1,Tables),
    fail.
everyGuestJustOneTable.
...
seuenJunts:-
    ...
    writeClause([-guestTable(G1,T),-guestTable(G2,T)]),
    fail.
    % ...
seuenJunts.
...
incompatibles:-
    ...
    cantSitTogether(G,X),
    table(T),
    atMost(1,[guestTable(G,T),guestTable(X,T)]),
    fail.
    % ...
incompatibles.
