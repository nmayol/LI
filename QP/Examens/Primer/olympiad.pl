...
mutuals(F1,F2):- student(F1), student(F2), friends(F1,F2), friends(F2,F1).
expertiseSameSubject(F1,F2):- student(F1), student(F2), expertise(F1,E1), expertise(F2,E2), E1 = E2.
...
expertiseDiffTopics:-
      ...
      expertiseSameSubject(S1,S2),
      atMost(1,[chosen(S1),chosen(S2)]),
      fail.
expertiseDiffTopics.

friendsWithEveryMember:-
      student(S1), student(S2), S1 < S2,
      not(friends(S1,S2)),
      atMost(1,[chosen(S1),chosen(S2)]),
      fail.
