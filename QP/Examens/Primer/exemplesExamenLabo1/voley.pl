...
noDoubles:-
    round(R), noDoubles(ND), member(R,ND),
    team(T),
    writeOneClause([-double(T,R)]),
    fail.
noDoubles.

atmostOneTVMatchPerRound:- 
    round(R),
    findall(match(S,T,R),(team(S), team(T), S \= T, tvMatch(S-T)),Lits),   
    atMost(1,Lits),
    fail.
atmostOneTVMatchPerRound.
...
relateHomeDouble:-
    ...
    R1 is R - 1,
    team(S),
    writeOneClause([-home(S,R),-home(S,R1),double(S,R)]),
    writeOneClause([home(S,R),home(S,R1),double(S,R)]),
    fail.
relateHomeDouble.
