...
incompatibleWorkers(W1,W2):- maxScore(N), workerScore(W1,S1), workerScore(W2,S2), N < S1 + S2.
...
eachTeamValidNumberIntegrants:-
      minSize(Min),
      maxSize(Max),
      team(T),
      findall(wt(W,T),worker(W),Integrants),
      atLeast(Min,Integrants),
      atMost(Max,Integrants),
      fail.
eachTeamValidNumberIntegrants.

notAPairExceedsMaxScore:-
      team(T),worker(W1),worker(W2), W1 \= W2, incompatibleWorkers(W1,W2),
