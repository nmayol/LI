...
noOverlapProfs:-
    professor(P), % per cada professor p
	course(C1), % per un curs 1
	course(C2), % per un altre curs 2
	C1 > C2, % comprovem que son diferents
	day(D), % per un dia d
	hour(H), % per una hora h
	writeClause([-cp(C1,P),-cp(C2,P),-cdh(C1,D,H),-cdh(C2,D,H)]), % enllacem professors amb classes (?)
	fail.
noOverlapProfs.

noOverlapRooms:- 
    room(R), % per cada habitacio
	course(C1), % per un curs 1
	course(C2), % per un altre curs 2
	C1 > C2, % comprovem que son diferents
	day(D), % per un dia d
	hour(H), % per una hora h
	writeClause([-cr(C1,R),-cr(C2,R),-cdh(C1,D,H),-cdh(C2,D,H)]), % enllacem habitacionss amb classes (?)
	fail.
noOverlapRooms.
...
maxProfs(MaxNumProf):- 
    findall(usedProf(P),professor(P),L),
	exactly(MaxNumProf,L).
maxProfs(_).
